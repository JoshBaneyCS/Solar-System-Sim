...
// ValidateAssets checks the asset directory structure and returns any errors
// and informational messages found.
func ValidateAssets(dir string) (errors []string, infos []string) {
	...
	for _, name := range []string{"sphere_32.glb", "sphere_64.glb"} {
		...
		if _, err := os.Stat(path); err != nil {
			infos = append(infos, fmt.Sprintf("optional mesh not found: %s (run 'make meshgen' to generate)", path))
		} else {
...
// BodyCount returns the number of bodies whose textures are validated.
func BodyCount() int {
	return len(bodies)
}
