...
pub static VEHICLES: &[Vehicle] = &[
    Vehicle {
        name: "Generic",
        stages: &[
            Stage { name: "Stage 1", isp: 290.0, thrust: 7e6, wet_mass: 400000.0, dry_mass: 30000.0 },
            Stage { name: "Stage 2", isp: 340.0, thrust: 1e6, wet_mass: 100000.0, dry_mass: 6000.0 },
        ],
    },
    Vehicle {
        name: "Falcon-like",
        stages: &[
            Stage { name: "Stage 1", isp: 282.0, thrust: 7.6e6, wet_mass: 433100.0, dry_mass: 25600.0 },
            Stage { name: "Stage 2", isp: 348.0, thrust: 934000.0, wet_mass: 111500.0, dry_mass: 4000.0 },
        ],
    },
    Vehicle {
        name: "Saturn V-like",
        stages: &[
            Stage { name: "S-IC", isp: 263.0, thrust: 35.1e6, wet_mass: 2290000.0, dry_mass: 131000.0 },
            Stage { name: "S-II", isp: 421.0, thrust: 5.141e6, wet_mass: 496200.0, dry_mass: 36200.0 },
            Stage { name: "S-IVB", isp: 421.0, thrust: 1.033e6, wet_mass: 123000.0, dry_mass: 13300.0 },
        ],
    },
];
...
pub static DESTINATIONS: &[Destination] = &[
    Destination { name: "LEO (200 km)", altitude: 200e3, apoapsis_alt: 200e3, semi_major_axis: 0.0, inclination: KSC_LATITUDE_RAD, frame: ReferenceFrame::EarthCentered },
    Destination { name: "ISS Orbit", altitude: 408e3, apoapsis_alt: 408e3, semi_major_axis: 0.0, inclination: 51.6 * PI / 180.0, frame: ReferenceFrame::EarthCentered },
    Destination { name: "GTO", altitude: 200e3, apoapsis_alt: GEO_ALTITUDE, semi_major_axis: 0.0, inclination: KSC_LATITUDE_RAD, frame: ReferenceFrame::EarthCentered },
    Destination { name: "Moon Transfer (TLI)", altitude: 200e3, apoapsis_alt: MOON_DISTANCE, semi_major_axis: 0.0, inclination: KSC_LATITUDE_RAD, frame: ReferenceFrame::EarthCentered },
    Destination { name: "Mars Transfer (Hohmann)", altitude: 200e3, apoapsis_alt: 0.0, semi_major_axis: MARS_ORBIT_SMA, inclination: 1.85 * PI / 180.0, frame: ReferenceFrame::Heliocentric },
];
...
pub fn plan(vehicle: &Vehicle, dest: &Destination) -> LaunchPlan {
    let parking_alt = if dest.altitude > 0.0 { dest.altitude } else { 200e3 };
    ...
    LaunchPlan {
        ...
        budget: DeltaVBudget { ascent, plane_change, transfer, arrival, total },
        transfer_time,
...
pub fn propagate_trajectory(launch_plan: &LaunchPlan, dest: &Destination) -> Trajectory {
    ...
    match dest.frame {
        ReferenceFrame::Heliocentric => {
            ...
            propagate_rk4(pos, vel, MU_SUN, 3600.0, launch_plan.transfer_time, ReferenceFrame::Heliocentric)
        }
        _ => {
            ...
            propagate_rk4(pos, vel, MU_EARTH, 60.0, duration, ReferenceFrame::EarthCentered)
        }
    }
}
...
fn propagate_rk4(
    ...
    let record_interval = if total_steps > 1000 { total_steps / 1000 } else { 1 };
    ...
    points.push(TrajectoryPoint { time: t, position: pos, velocity: vel });
...
fn rk4_step(pos: [f64; 3], vel: [f64; 3], dt: f64, mu: f64) -> ([f64; 3], [f64; 3]) {
    ...
    let new_pos = add(pos, scale(add(add(k1v, scale(v2, 2.0)), add(scale(v3, 2.0), v4)), dt / 6.0));
    let new_vel = add(vel, scale(add(add(k1a, scale(a2, 2.0)), add(scale(a3, 2.0), a4)), dt / 6.0));
...
pub fn summary(plan: &LaunchPlan) -> String {
    let status = if plan.feasible { "FEASIBLE" } else { "NOT FEASIBLE (insufficient dv)" };
    ...
    format!(
        ...
        plan.vehicle_name, plan.vehicle_delta_v, plan.vehicle_delta_v / 1000.0,
        plan.dest_name,
        status,
        plan.budget.ascent, plan.budget.ascent / 1000.0,
        plan.budget.plane_change, plan.budget.plane_change / 1000.0,
        plan.budget.transfer, plan.budget.transfer / 1000.0,
        plan.budget.arrival, plan.budget.arrival / 1000.0,
        plan.budget.total, plan.budget.total / 1000.0,
        plan.parking_altitude / 1000.0,
        plan.parking_orbit_v, plan.parking_orbit_v / 1000.0,
        transfer_days, plan.transfer_time / 3600.0,
    )
}
